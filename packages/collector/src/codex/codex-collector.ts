...
import { conversationFingerprint, mapRolloutToEvents } from "./event-mapping";
import {
	parseRolloutLine,
	type RolloutLine,
	...
	async collect({
		...
		for (const filePath of files) {
			try {
				...
				if (events.length > 0) {
					// Fingerprint the FULL parsed rollout, not the `since`-filtered
					// events: a resumed copy and its original must line up on
					// their shared history for the prefix test to see the overlap.
					sessions.push({
						startTimestamp,
						events,
